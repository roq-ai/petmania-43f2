...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiFeather, FiHome } from 'react-icons/fi';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['pro', 'starter', 'gainer'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<PetopiaInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/petopias` : null),
    () => getPetopias({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Petopia',
      path: isTenantUser ? `/petopias/view/${data?.[0]?.id}` : '/petopias',
      entity: 'petopia',
